...
def evaluate_diffusion_policy(model_path, dataset_path, num_episodes=1):
    ...
    for episode in range(num_episodes):
        ...
        observation_env = sample['observation.env'].unsqueeze(0).to(device)  # torch.Size([1, 3, 100, 100])
        # observation_state = sample['observation.state'].unsqueeze(0).to(device)  # torch.Size([1, 2])

        test_state = [[0, 0]]
        observation_state = torch.tensor(test_state).unsqueeze(0).to(device)
        ...
        with torch.inference_mode():
            predicted_actions = policy.select_action(test_observation)
