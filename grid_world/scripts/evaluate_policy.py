...
def evaluate_diffusion_policy(model_path, dataset_path, num_episodes=1):
    ...
    for episode in range(num_episodes):
        ...
        observation_env = sample['observation.env'].to(device)  # Add batch dimension
        observation_state = sample['observation.state'].to(device)
        ...
        plt.scatter(predicted_actions[:, 0], predicted_actions[:, 1], c='red', label='Predicted Trajectory')
